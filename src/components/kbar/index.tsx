  ...
  const actions = useMemo(
    () =>
      navItems.flatMap((navItem) => {
        ...
        const childActions =
          navItem.items?.map(
            (childItem: { title: string; shortcut: any; url: string }) => ({
        ...
        <KBarPositioner className="scrollbar-hide fixed inset-0 z-[99999] bg-black/80  !p-0 backdrop-blur-sm">
          <KBarAnimator className="relative !mt-64 w-full max-w-[600px] !-translate-y-12 overflow-hidden rounded-lg border bg-background text-foreground shadow-lg">
            <div className="bg-background">
              <div className="border-b-2 border-x-0">
                <KBarSearch className="w-full px-6 py-4 text-lg border-none outline-none bg-background focus:outline-none focus:ring-0 focus:ring-offset-0" />
              </div>
              <RenderResults />
            </div>
          </KBarAnimator>
